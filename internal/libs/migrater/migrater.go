...
func Auto(ctx context.Context, logger zerolog.Logger, db *sql.DB, dir string, fileSystem fs.FS) error {
	...
	err = m.Migrate(ctx, dir, migrate.Config{Cmd: migrate.Up}, migrate.WithCustomFS(fileSystem))
	if err != nil {
